...
- [x] **7.1 Schema Database Avanzato**: Creare una tabella `knowledge_chunks` per salvare pezzi di codice, ricette e snippet estratti dall'Analyst.
- [x] **7.2 Potenziamento Analyst (Extraction)**: Modificare il prompt di Llama 3.2 per estrarre non solo nomi di entità ma blocchi di contenuto tecnico/pratico rilevanti per il Goal.
- [x] **7.3 Synthesizer Iterativo (Chapter-by-Chapter)**: Riscrivere il nodo Synthesizer affinché esegua una chiamata a Gemini per ogni capitolo dell'outline, garantendo massima prolissità e dettaglio.
- [x] **7.4 Parametro `detail_level`**: Aggiungere il controllo nel `config.json` per regolare la profondità della sintesi.
